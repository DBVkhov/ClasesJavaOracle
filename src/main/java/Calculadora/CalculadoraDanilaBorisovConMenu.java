...
public class CalculadoraDanilaBorisovConMenu {
    ...
    public static void main(String[] args) throws IOException {
        ...
        do {
            ...
            switch (option) {
                ...
                case 5 -> potencia(number1, number2);
    ...
    private static void potencia(float val1, float val2){
        if(val2==0){
            System.out.println(val1 + "^(" + val2 + ")= " + 1);
        }
        else if(val2>0) {
            int contador = 1;
            float resultado = val1;
            while (contador < val2) {
                resultado = resultado * val1;
                contador++;
            }
            System.out.println(val1 + "^(" + val2 + ")= " + resultado);
        }
        else{
            int contador = 1;
            float resultado = val1;
            while (contador < (-1*val2)){
                resultado = resultado * val1;
                contador++;
            }
            System.out.println(val1 + "^(" + val2 + ")= " + 1/resultado);
        }
    }
